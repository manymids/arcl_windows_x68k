...
#ifdef ARCL_WATCHPOINT
/* ARCL (windows/arcl_watchpoint.c): memory watchpoint hook, added for the
 * Windows ARCL frontend's L2 debug layer. See x68k_mcp.md 0.1.1 for why
 * this exists and the constraints it was written under (minimal diff,
 * behavior/codegen unchanged when ARCL_WATCHPOINT is undefined). */
...
static void wm_cnt(uint32_t addr, uint8_t val)
{
	addr &= 0x00ffffff;
#ifdef ARCL_WATCHPOINT
	arcl_watchpoint_on_write(addr, val);
#endif
...
#ifdef ARCL_WATCHPOINT
	{
		uint8_t v = MEM[addr ^ 1];
		arcl_watchpoint_on_read(addr, v);
		return v;
	}
#else
		return MEM[addr ^ 1];
...
#ifdef ARCL_WATCHPOINT
	{
		uint8_t v = MemReadTable[(addr >> 13) & 0xff](addr);
		arcl_watchpoint_on_read(addr, v);
		return v;
	}
#else
